...
of the deque with approximately the same  performance in either direction.
...
def main():
    ...
    try:
        ...
        if n <= 0 or n > 100:
            raise RuntimeError()
        d = deque()
        whitelist = ['append', 'appendleft', 'pop', 'popleft']
        for i in range(0, n):
            s = input().split()
            if s[0] in whitelist:
                # it's either a command or command and int
                if len(s) == 2:
                    cmd, val = s
                    eval('d.{}({})'.format(cmd, int(val)))
                elif len(s) == 1:
                    cmd = s[0]
                    eval('d.{}()'.format(cmd))
        for value in d:
        ...
        print("The number you entered should be 0 < N <= 100")
